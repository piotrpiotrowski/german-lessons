...
export class PastSimpleVerbsConjunctionsService implements FinderService<TrainingRowModel> {
  ...
  private finderService = new CsvFinderService<TrainingRowModel>(rawPastSimpleVerbsConjunctions, columns => this.mapToTrainingRowModel(columns));

  find = (predicate: Predicate<TrainingRowModel>) => this.finderService.find(predicate);

  private mapToTrainingRowModel = (columns) =>
    new TrainingRowModel(new Map<Language, string>([[Language.ENGLISH, columns[0]], [Language.POLISH, columns[1]], [Language.GERMAN, this.extractVerb(columns[2])]]),
      +columns[10],
      [
        new Answer('i', this.extractVerb(columns[4])),
        new Answer('youSingular', this.extractVerb(columns[5])),
        new Answer('heSheIt', this.extractVerb(columns[6])),
        new Answer('we', this.extractVerb(columns[7])),
        new Answer('youPlural', this.extractVerb(columns[8]))
      ])
